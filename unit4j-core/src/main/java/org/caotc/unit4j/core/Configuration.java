...
public final class Configuration implements WithId {
   ...
   */
  @NonNull
  final Table<? super AliasRegistrable, Alias.Type, Alias> aliasRegistrableToTypeToAliasTable = Tables
      .synchronizedTable(HashBasedTable.create());
  /**
   * 别名值与别名类型和对应的可注册别名的对象 数据与{@link #aliasRegistrableToTypeToAliasTable}保持对应
   */
  @NonNull
...
//    registerAlias(UnitConstant.SIEVERT, Alias.SIEVERT_ENGLISH_NAME, Alias.SIEVERT_SYMBOL);
  ...
  public ImmutableSet<Alias> aliases(@NonNull AliasRegistrable aliasRegistrable) {
    return ImmutableSet.copyOf(aliasRegistrableToTypeToAliasTable.row(aliasRegistrable).values());
  }
  ...
  public Optional<Alias> alias(@NonNull AliasRegistrable aliasRegistrable,
    ...
    return Optional.ofNullable(aliasRegistrableToTypeToAliasTable.get(aliasRegistrable, aliasType));
  }
  ...
  public synchronized Configuration registerAlias(@NonNull AliasRegistrable aliasRegistrable,
      @NonNull Alias alias) {
    if (aliasRegistrableToTypeToAliasTable.contains(aliasRegistrable, alias.type())) {
      throw new IllegalArgumentException("" + aliasRegistrable + alias + "重复注册");
    }
    aliasRegistrableToTypeToAliasTable.put(aliasRegistrable, alias.type(), alias);
