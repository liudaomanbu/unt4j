 ...
 */
@Value
public class ReadableWithUnitProperty<O, P> extends BaseWithUnitProperty<O, P, ReadableProperty<O, P>> implements ReadableProperty<O, Quantity> {
    public ReadableWithUnitProperty(@NonNull ReadableProperty<O, P> delegate, @NonNull Configuration configuration, @NonNull MathContext mathContext) {
    ...
    public @NonNull <O1> ReadableProperty<O1, Quantity> ownerType(@NonNull TypeToken<O1> ownerType) {
        return new ReadableWithUnitProperty<>(delegate().ownerType(ownerType), configuration(), mathContext());
    }
}
